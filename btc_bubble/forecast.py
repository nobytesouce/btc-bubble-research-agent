...
def write_24_hour_chart(path: str | Path, forecasts: pd.DataFrame) -> Path:
    ...
    fig, price_axis = plt.subplots(figsize=(15, 7.5))
    bubble_axis = price_axis.twinx()
    price_axis.plot(actual_dates, forecasts["price_at_forecast"], color="#2563eb", linewidth=2.0, label="BTC price on target day")
    bubble_axis.plot(actual_dates, forecasts["predicted_next_24h_mean_bubble_usd"] / 1_000_000, color="#f59e0b", linewidth=2.2, label="Predicted average for target day")
    ...
    price_axis.set_title("BTC one-day-ahead bubble forecast aligned by target day")
    price_axis.set_xlabel("Predicted target date (UTC)")
    price_axis.set_ylabel("BTC price (USDT)", color="#2563eb")
    bubble_axis.set_ylabel("Average qualifying bubble notional (USD millions)")
    price_axis.grid(alpha=0.2)
    price_axis.xaxis.set_major_locator(mdates.WeekdayLocator(interval=1))
